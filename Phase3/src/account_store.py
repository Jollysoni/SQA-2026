...
class AccountStore:
    ...
    def is_disabled(self, account_number):
        """
        True if the account exists and is disabled (status 'D').
        If account does not exist, return True to block actions safely.
        """
    ...
    def get_balance(self, account_number):
        """
        Return the current in-memory balance, or None if not found.
        """
        acct = self.find_account(account_number)
        if acct is None:
            return None
    ...
    def set_balance(self, account_number, new_balance):
        """
        Update in-memory balance for an account. If not found, do nothing.
        """
        acct = self.find_account(account_number)
        if acct is None:
            return
